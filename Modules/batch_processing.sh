...
source ../Horn_Task/venv/Scripts/activate
...
SRC="C:\Users\lp3200\Columbia University Irving Medical Center\Remote DAYC-2 and Maternal Cognition - Horncut_Highchair_Videos\Testing"
DEST_PROCESSED="C:\Users\lp3200\Columbia University Irving Medical Center\Remote DAYC-2 and Maternal Cognition - Horncut_Highchair_Videos\Testing\Testing_output"
...
while [ ${#files[@]} -gt 0 ]; do
    ...
    get_next_batch files batch "$BATCH_SIZE" #now batch and files both exist as arrays and batch is progressively updated every iteration
    
    # Download locally by forcing OneDrive to sync them
    local_download batch

    # Process batch of locally downloaded files (example: run Python script on each file)
    python code/Horn_detection_pipeline_luis.py "$BATCH" "$REF"
   
    exit 1


    while IFS= read -r file; do #read one line from input and store it in file variable
        base=$(basename "$file")
        name="${base%.*}"
